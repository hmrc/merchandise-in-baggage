...
trait CustomsDutyCalculator {
  private[service] val findCurrencyRate: (String, LocalDate) => Future[List[CurrencyConversionResponse]]

  def customDuty(calculationRequest: CalculationRequest)(implicit ec: ExecutionContext): EitherT[Future, BusinessError, Amount] =
    for {
